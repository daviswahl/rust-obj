...
use tokio_proto::multiplex::{RequestId, ClientProto, ServerProto, ClientService};
use std::io;
...
impl Decoder for CacheCodec {
    ...
    fn decode(&mut self, buf: &mut BytesMut) -> Result<Option<(RequestId, Message)>, io::Error> {
        ...
        let key = &message[header_size..header_size + key_len];
        let data = &message[header_size + key_len..header_size + key_len + payload_len];
        Ok(Some((
